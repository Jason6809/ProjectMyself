export default function ProjectSection() {
  return (
    <section id='projectSection'>
      <div className='container-fluid pt-5 min-vh-100'>
        <div className='row'>
          <div className='col'>
            <h1 className='display-4 mb-5'>My Projects</h1>
          </div>
        </div>

        <div className='jumbotron custom-jumbotron'>
          <div className='row'>
            <div className='col'>
              <div className='custom-p-box bg-blue-gray py-4 px-4'>
                <p className='lead custom-font-weight-503 m-0'>
                  Online Admission System (Part of SUC Campus Management System)
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499 custom-height-23px'>
                  SUC Project
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499'>
                  Web Application
                </p>
              </div>
            </div>
          </div>

          <hr className='w-100 custom-hr' />

          <div className='row'>
            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0'>
                    Core Technologies:
                  </p>
                </li>
              </ul>

              <div className='container-fluid'>
                <div className='row align-items-center text-center'>
                  <div className='col-12 col-md-6'>
                    <img src='https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Ruby_On_Rails_Logo.svg/1200px-Ruby_On_Rails_Logo.svg.png' className='img-fluid px-3' />
                  </div>

                  <div className='col-12 col-md-6'>
                    <img src='https://www.pngitem.com/pimgs/m/197-1973694_transparent-png-postgresql-logo-png-download.png' className='img-fluid px-3' />
                  </div>

                  <div className='col-12 col-md-6'>
                    <img src='https://miro.medium.com/max/3600/1*fIjRtO5P8zc3pjs0E5hYkw.png' className='img-fluid px-3' />
                  </div>
                </div>
              </div>
            </div>

            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0 w-100'>
                    Links:
                  </p>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='http://suc-nova-staging.herokuapp.com/'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    Official Open BETA
                  </a>
                </li>

                <li>
                  <a
                    target='_blank'
                    href='http://admission.southern.edu.my/'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    Official Live Portal
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className='jumbotron custom-jumbotron'>
          <div className='row'>
            <div className='col'>
              <div className='custom-p-box bg-blue-gray py-4 px-4'>
                <p className='lead custom-font-weight-503 m-0'>
                  Onsite Stylist Finding, Calling and Booking System
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499 custom-height-23px'>
                  Final Year Project
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499'>
                  Android Application
                </p>
              </div>
            </div>
          </div>

          <hr className='w-100 custom-hr' />

          <div className='row'>
            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0'>
                    Core Technologies:
                  </p>
                </li>
              </ul>

              <div className='container-fluid'>
                <div className='row align-items-center text-center'>
                  <div className='col-12 col-md-6'>
                    <img src='https://braze-marketing-assets.s3.amazonaws.com/images/partner_logos/react-native.png' className='img-fluid px-3' />
                  </div>

                  <div className='col-12 col-md-6'>
                    <img src='https://1.bp.blogspot.com/-YIfQT6q8ZM4/Vzyq5z1B8HI/AAAAAAAAAAc/UmWSSMLKtKgtH7CACElUp12zXkrPK5UoACLcB/s1600/image00.png' className='img-fluid px-3' />
                  </div>
                </div>
              </div>
            </div>

            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0 w-100'>
                    Links:
                  </p>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='https://firebasestorage.googleapis.com/v0/b/jason-tam.appspot.com/o/venus-client-release.apk?alt=media&token=782a1d99-3c23-4d87-8319-a19430d53517'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    Client APK Download
                  </a>
                </li>

                <li>
                  <a
                    target='_blank'
                    href='https://github.com/Jason6809/VenusClient'
                    className='lead custom-font-weight-300 custom-font-size-1-15-rem m-0 px-3 btn btn-sm btn-dark w-100'>
                    GitHub
                  </a>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='https://firebasestorage.googleapis.com/v0/b/jason-tam.appspot.com/o/venus-stylist-release.apk?alt=media&token=c9fc3c9c-158b-407a-8ff9-1a9dcf390a3d'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    Stylist APK Download
                  </a>
                </li>

                <li>
                  <a href='https://github.com/Jason6809/VenusStylist' target='_blank' className='lead custom-font-weight-300 custom-font-size-1-15-rem m-0 px-3 btn btn-sm btn-dark w-100'>
                    GitHub
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className='jumbotron custom-jumbotron'>
          <div className='row'>
            <div className='col'>
              <div className='custom-p-box bg-blue-gray py-4 px-4'>
                <p className='lead custom-font-weight-503 m-0'>
                  Employee Recognition System
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499 custom-height-23px'>
                  Campus Competition Project
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499'>
                  Web Application
                </p>
              </div>
            </div>
          </div>

          <hr className='w-100 custom-hr' />

          <div className='row'>
            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0'>
                    Core Technologies:
                  </p>
                </li>
              </ul>

              <div className='container-fluid'>
                <div className='row align-items-center text-center'>
                  <div className='col-12 col-md-6'>
                    <img src='https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/AngularJS_logo.svg/695px-AngularJS_logo.svg.png' className='img-fluid px-3' />
                  </div>

                  <div className='col-12 col-md-6'>
                    <img src='https://1.bp.blogspot.com/-YIfQT6q8ZM4/Vzyq5z1B8HI/AAAAAAAAAAc/UmWSSMLKtKgtH7CACElUp12zXkrPK5UoACLcB/s1600/image00.png' className='img-fluid px-3' />
                  </div>
                </div>
              </div>
            </div>

            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0 w-100'>
                    Links:
                  </p>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='https://recognition-modules.web.app/'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    View Client Demo
                  </a>
                </li>

                <li>
                  <a
                    target='_blank'
                    href='https://github.com/Jason6809/ProjectTicMe'
                    className='lead custom-font-weight-300 custom-font-size-1-15-rem m-0 px-3 btn btn-sm btn-dark w-100'>
                    GitHub
                  </a>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='https://admin-recognition-modules.web.app/'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    View Admin Demo
                  </a>
                </li>

                <li>
                  <a
                    href='https://github.com/Jason6809/ProjectTicMe-Admin'
                    target='_blank'
                    className='lead custom-font-weight-300 custom-font-size-1-15-rem m-0 px-3 btn btn-sm btn-dark w-100'>
                    GitHub
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className='jumbotron custom-jumbotron'>
          <div className='row'>
            <div className='col'>
              <div className='custom-p-box bg-blue-gray py-4 px-4'>
                <p className='lead custom-font-weight-503 m-0'>
                  Hair Saloon Management System
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499 custom-height-23px'>
                  Course Assignment
                </p>

                <p className='m-0 custom-text-gray custom-font-size-1-15-rem custom-font-weight-499'>
                  Web Application
                </p>
              </div>
            </div>
          </div>

          <hr className='w-100 custom-hr' />

          <div className='row'>
            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0'>
                    Core Technologies:
                  </p>
                </li>
              </ul>

              <div className='container-fluid'>
                <div className='row align-items-center text-center'>
                  <div className='col-12 col-md-6'>
                    <img src='https://www.php.net/images/logos/new-php-logo.png' className='img-fluid px-3' />
                  </div>

                  <div className='col-12 col-md-6'>
                    <img src='https://download.logo.wine/logo/MySQL/MySQL-Logo.wine.png' className='img-fluid px-3' />
                  </div>
                </div>
              </div>
            </div>

            <div className='col-12 col-lg-6'>
              <ul className='custom-list-style-none'>
                <li>
                  <p className='lead custom-font-weight-499 custom-font-size-1-15-rem m-0 w-100'>
                    Links:
                  </p>
                </li>
              </ul>

              <ul className='custom-list-style-none'>
                <li>
                  <a
                    target='_blank'
                    href='http://gemini-salon.epizy.com'
                    className='lead btn btn-sm bg-dark-blue custom-font-weight-300 custom-font-size-1-15-rem mb-2 px-3 w-100'>
                    View Demo
                  </a>
                </li>

                <li>
                  <a
                    target='_blank'
                    href='https://github.com/Jason6809/ProjectGemini'
                    className='lead custom-font-weight-300 custom-font-size-1-15-rem m-0 px-3 btn btn-sm btn-dark w-100'>
                    GitHub
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
